<link rel="stylesheet" type="text/css" href="{{asset('assets-tw/vendor/libs/jstree/css/jstree.min.css')}}">
<link rel="stylesheet" type="text/css" href="{{asset('assets-tw/vendor/libs/jstree/css/ext-component-tree.min.css')}}">
...
<script src="{{asset('assets-tw/vendor/libs/jstree/js/jstree.min.js')}}"></script>
<script src="{{asset('assets-tw/vendor/libs/jstree/js/ext-component-tree.min.js')}}"></script>
<script>
(function () {
    // Enhance the child-user multiselect with erpSearchSelect (replaces select2).
    // Guarded: if the component isn't loaded on this host page, the native
    // <select multiple> is left as-is.
    function initChildSelect() {
        if (typeof erpSearchSelect !== 'function') return;
        var el = document.getElementById('child_id');
        if (!el || (el._erpSelectWrapper && el._erpSelectWrapper.parentNode)) return;
        // Flatten optgroups, prefixing the group name (the component does not
        // render <optgroup> headers). The native <select> keeps its groups.
        var opts = [];
        Array.prototype.forEach.call(el.querySelectorAll('option'), function (o) {
            if (!o.value) return;
            var grp = (o.parentElement && o.parentElement.tagName === 'OPTGROUP')
                ? o.parentElement.getAttribute('label') : '';
            opts.push({ value: String(o.value), label: (grp ? grp + ' — ' : '') + o.textContent.trim() });
        });
        erpSearchSelect(el, { multiple: true, options: opts, placeholder: 'Select users' });
    }
    initChildSelect();
})();
</script>