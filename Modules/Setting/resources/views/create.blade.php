...
@section('title', __('setting::message.create_setting'))
@section('nav-module', 'setting')
@section('breadcrumb', 'Home > Setting > Create')

@section('content')
{{-- Page Header --}}
<div class="flex items-center justify-between mb-6">
    <h1 class="text-xl font-bold text-zinc-900">{{ __('setting::message.create_setting') }}</h1>
    ...
    <form action="javascript:void(0);" id="form" method="POST" autocomplete="none" enctype="multipart/form-data" novalidate>
        ...
        <div class="p-5 border-t border-zinc-200">
            ...
            <textarea name="address" id="address" rows="3"
                      class="w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 focus:ring-1 focus:ring-zinc-500 focus:border-zinc-500"
                      placeholder="{{ __('setting::message.address') }}">{{ old('address') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <div class="mt-2 flex items-center justify-center rounded-md p-2" style="background-color: var(--erp-bg);">
                    <img id="logo_preview" class="rounded object-contain" style="max-height: 5rem;" src="{{ asset('assets/img/avatars/1.png') }}">
                </div>
            </div>
            {{-- Logo Dark --}}
            <div>
                ...
                <div class="mt-2 flex items-center justify-center rounded-md p-2" style="background-color: var(--erp-text);">
                    <img id="logo_dark_preview" class="rounded object-contain" style="max-height: 5rem;" src="{{ asset('assets/img/avatars/1.png') }}">
                </div>
            </div>
            {{-- Favicon --}}
            <div>
                ...
                <div class="mt-2 flex items-center justify-center rounded-md p-2">
                    <img id="favicon_preview" class="rounded" style="max-height: 45px; max-width: 45px;" src="{{ asset('assets/img/avatars/1.png') }}">
                </div>
            </div>
        </div>
        </div>
    ...
    $(document).on('click', '.save-settings', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $btn.html('<i class="fa-solid fa-check mr-1.5 text-xs"></i> {{ __("message.common.save") }}').attr('disabled', false);
