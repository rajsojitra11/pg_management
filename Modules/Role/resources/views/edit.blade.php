...
@section('breadcrumb', __('role::message.edit'))

@section('content')
<div class="flex items-center justify-between mb-6">
    <div>
        <h5 class="text-lg font-semibold text-zinc-900">{{ __('role::message.edit') }} — {{ str_replace('_', ' ', $role->name) }}</h5>
        <p class="text-sm mt-0.5" style="color: var(--erp-text-secondary);">Modify permissions for this role</p>
    </div>
    @can('role-list')
...
<form action="{{ route('roles.update', $role->id) }}" method="POST" id="roleForm" novalidate>
    @csrf
    @method('PATCH')
    ...
    <div class="rounded-lg border border-zinc-200 bg-white shadow-sm mb-4">
        <div class="p-5">
            ...
            <div class="flex items-start gap-3 p-3 rounded-md border border-zinc-200 bg-zinc-50 mb-4">
                <input type="checkbox" name="all_years" id="allYears" value="1" class="mt-0.5 h-4 w-4 rounded border-zinc-300 text-zinc-900 focus:ring-zinc-900" {{ $role->yearAccess?->all_years ? 'checked' : '' }}>
                <div class="flex-1">
            ...
            <div id="yearsBoxWrap">
                ...
                <input type="number" name="allowed_year" id="yearsBox" min="1" step="1" inputmode="numeric" autocomplete="off"
                    placeholder="e.g., 1" value="{{ old('allowed_year', $role->yearAccess?->allowed_year) }}"
    ...
    <div class="rounded-lg border border-zinc-200 bg-white shadow-sm mb-4">
        <div class="p-5">
            @include('role::partials.permission-grid', ['permission' => $permission, 'rolePermissions' => $rolePermissions])
        </div>
    </div>
...
$(document).ready(function() {
    ...
    (function initYearAccess() {
        if ($('#allYears').is(':checked')) {
    ...
    // On load: trigger change to set parent/selectAll state from pre-checked boxes
    $('.child-checkbox').each(function() { $(this).trigger('change'); });
