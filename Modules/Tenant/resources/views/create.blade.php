...
@section('title', __('tenant::message.add'))
@section('nav-module', 'tenant')
@section('breadcrumb', __('tenant::message.add'))

@section('content')
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-xl sm:text-2xl font-bold text-zinc-900">{{ __('tenant::message.add') }}</h1>
        <p class="text-sm text-zinc-500 mt-1">{{ __('tenant::message.create_new_tenant') }}</p>
    </div>
    @can('tenant-list')
...
<form action="{{ route('tenant.store') }}" method="POST" id="tenantForm" enctype="multipart/form-data" novalidate class="w-full">
    @csrf
    ...
    <div class="rounded-lg border border-zinc-200 bg-white shadow-sm overflow-hidden w-full">
        ...
        <div class="flex items-center gap-2 px-4 py-1.5 border-b" style="background:#3D52A0; border-bottom-color:#324690;">
            <div class="h-6 w-6 rounded flex items-center justify-center" style="background:rgba(255,255,255,.18);">
                <i class="fa-solid fa-user-plus text-white" style="font-size:11px;"></i>
            </div>
            <h2 class="text-sm font-semibold text-white">{{ __('tenant::message.tenant_registration') }}</h2>
        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div>
                            ...
                            <input type="email" name="email" id="email" required
                                   class="h-9 w-full rounded-md border border-zinc-200 bg-transparent px-3 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2"
                                   placeholder="{{ __('tenant::message.enter_email') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                        ...
                        <div>
                            ...
                            <select name="gender" id="gender"
                                ...
                                <option value="Male">{{ __('tenant::message.male') }}</option>
                                <option value="Female">{{ __('tenant::message.female') }}</option>
                                <option value="Other">{{ __('tenant::message.other') }}</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div>
                            ...
                            <select name="payment_method" id="payment_method"
                                    class="h-9 w-full rounded-md border border-zinc-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2">
                                <option value="">{{ __('message.common.select') }}</option>
                                <option value="Cash">{{ __('tenant::message.cash') }}</option>
                                <option value="Bank Transfer">{{ __('tenant::message.bank_transfer') }}</option>
                                <option value="Cheque">{{ __('tenant::message.cheque') }}</option>
                                <option value="UPI">{{ __('tenant::message.upi') }}</option>
                                <option value="Other">{{ __('tenant::message.other') }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                        <div>
                            ...
                            <select name="id_proof_type" id="id_proof_type" required
                                    class="h-9 w-full rounded-md border border-zinc-200 bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2">
                                <option value="">{{ __('message.common.select') }}</option>
                                <option value="Aadhar Card">{{ __('tenant::message.aadhar_card') }}</option>
                                <option value="PAN Card">{{ __('tenant::message.pan_card') }}</option>
                                <option value="Passport">{{ __('tenant::message.passport') }}</option>
                                <option value="Driving License">{{ __('tenant::message.driving_license') }}</option>
                                <option value="Voter ID">{{ __('tenant::message.voter_id') }}</option>
                                <option value="Other">{{ __('tenant::message.other') }}</option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 mb-1.5">{{ __('tenant::message.id_proof_file') }} <span class="text-red-500">*</span></label>
                            <input type="file" name="id_proof_file" id="id_proof_file" accept=".jpg,.jpeg,.png,.pdf" required
                ...
                <div data-wizard-panel style="display:none;" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="permanent_address" id="permanent_address" rows="3"
                                  ...
                                  placeholder="{{ __('tenant::message.enter_permanent_address') }}"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="additional_notes" id="additional_notes" rows="2"
                                  class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2"
                                  placeholder="{{ __('tenant::message.enter_additional_notes') }}"></textarea>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    function updateReviewSection() {
        ...
        document.querySelector('.review-id-proof-file').textContent = idFile && idFile.files && idFile.files.length > 0 ? idFile.files[0].name : '-';
