...
@section('title', __('lang.labels.env_variable.create'))
@section('nav-module', 'envvariable')
@section('breadcrumb', 'Home > Env Variables > Create')

@section('content')
{{-- Page Header --}}
<div class="flex items-center justify-between mb-6">
    <h5 class="text-lg font-semibold text-zinc-900">{{ __('lang.labels.env_variable.create') }}</h5>
...
<form action="javascript:void(0);" method="POST" id="form" novalidate>
    ...
    <div class="rounded-lg border border-zinc-200 bg-white shadow-sm">
            ...
            <div class="grid grid-cols-12 gap-4">
                ...
                <div class="col-span-12 sm:col-span-6">
                    ...
                    <select id="type" name="type" required
                        ...
                        <option value="text" {{ old('type') == 'text' ? 'selected' : '' }}>{{ __('lang.labels.env_variable.type_text') }}</option>
                        <option value="number" {{ old('type') == 'number' ? 'selected' : '' }}>{{ __('lang.labels.env_variable.type_number') }}</option>
                        <option value="boolean" {{ old('type') == 'boolean' ? 'selected' : '' }}>{{ __('lang.labels.env_variable.type_boolean') }}</option>
                        <option value="select" {{ old('type') == 'select' ? 'selected' : '' }}>{{ __('lang.labels.env_variable.type_select') }}</option>
                        <option value="password" {{ old('type') == 'password' ? 'selected' : '' }}>{{ __('lang.labels.env_variable.type_password') }}</option>
                    </select>
                ...
                <div class="col-span-12 sm:col-span-6 flex items-end">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="erp-toggle relative cursor-pointer" style="width:36px;height:20px;background:var(--erp-primary);border-radius:9999px;transition:background-color 0.2s;"
                            ...
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }} class="absolute" style="opacity:0;width:0;height:0;">
                ...
                <div class="col-span-12" id="options_field" style="display: none;">
                    ...
                    <textarea id="options" name="options" rows="3"
                              ...
                              class="w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 placeholder:text-zinc-400 focus:ring-1 focus:ring-zinc-500 focus:border-zinc-500">{{ old('options') ? json_encode(old('options')) : '' }}</textarea>
                ...
                <div class="col-span-12">
                    ...
                    <input type="text" id="validation_rules" name="validation_rules" value="{{ old('validation_rules') }}"
                ...
                <div class="col-span-12">
                    ...
                    <textarea id="description" name="description" rows="2"
                              ...
                              class="w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 placeholder:text-zinc-400 focus:ring-1 focus:ring-zinc-500 focus:border-zinc-500">{{ old('description') }}</textarea>
            ...
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-12 sm:col-span-6 lg:col-span-3">
                    <div class="flex items-center gap-3">
                        <div class="erp-toggle relative cursor-pointer" style="width:36px;height:20px;background:var(--erp-border);border-radius:9999px;transition:background-color 0.2s;"
                             onclick="var cb=this.querySelector('input');cb.checked=!cb.checked;this.style.backgroundColor=cb.checked?'var(--erp-primary)':'var(--erp-border)';this.querySelector('.erp-toggle-dot').style.transform=cb.checked?'translateX(16px)':'translateX(0)';">
                            <input type="checkbox" name="is_encrypted" id="is_encrypted" value="1" {{ old('is_encrypted') ? 'checked' : '' }} class="absolute" style="opacity:0;width:0;height:0;">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-zinc-400">{{ __('envvariable::message.encryption_help') }}</p>
                        </div>
                    </div>
                </div>
                <div class="col-span-12 sm:col-span-6 lg:col-span-3">
                    <div class="flex items-center gap-3">
                        <div class="erp-toggle relative cursor-pointer" style="width:36px;height:20px;background:var(--erp-border);border-radius:9999px;transition:background-color 0.2s;"
                             onclick="var cb=this.querySelector('input');cb.checked=!cb.checked;this.style.backgroundColor=cb.checked?'var(--erp-primary)':'var(--erp-border)';this.querySelector('.erp-toggle-dot').style.transform=cb.checked?'translateX(16px)':'translateX(0)';">
                            <input type="checkbox" name="is_sensitive" id="is_sensitive" value="1" {{ old('is_sensitive') ? 'checked' : '' }} class="absolute" style="opacity:0;width:0;height:0;">
                            <div class="erp-toggle-dot absolute bg-white rounded-full shadow" style="width:16px;height:16px;top:2px;left:2px;transition:transform 0.2s;transform:translateX(0);"></div>
                        </div>
                        <div>
                ...
                <div class="col-span-12 sm:col-span-6 lg:col-span-3">
                    <div class="flex items-center gap-3">
                        <div class="erp-toggle relative cursor-pointer" style="width:36px;height:20px;background:var(--erp-primary);border-radius:9999px;transition:background-color 0.2s;"
                             onclick="var cb=this.querySelector('input');cb.checked=!cb.checked;this.style.backgroundColor=cb.checked?'var(--erp-primary)':'var(--erp-border)';this.querySelector('.erp-toggle-dot').style.transform=cb.checked?'translateX(16px)':'translateX(0)';">
                            <input type="checkbox" name="is_editable" id="is_editable" value="1" {{ old('is_editable', '1') ? 'checked' : '' }} class="absolute" style="opacity:0;width:0;height:0;">
                            <div class="erp-toggle-dot absolute bg-white rounded-full shadow" style="width:16px;height:16px;top:2px;left:2px;transition:transform 0.2s;transform:translateX(16px);"></div>
                        </div>
                        <div>
                ...
                <div class="col-span-12 sm:col-span-6 lg:col-span-3">
                    <div class="flex items-center gap-3">
                        <div class="erp-toggle relative cursor-pointer" style="width:36px;height:20px;background:var(--erp-border);border-radius:9999px;transition:background-color 0.2s;"
                             onclick="var cb=this.querySelector('input');cb.checked=!cb.checked;this.style.backgroundColor=cb.checked?'var(--erp-primary)':'var(--erp-border)';this.querySelector('.erp-toggle-dot').style.transform=cb.checked?'translateX(16px)':'translateX(0)';">
                            <input type="checkbox" name="requires_restart" id="requires_restart" value="1" {{ old('requires_restart') ? 'checked' : '' }} class="absolute" style="opacity:0;width:0;height:0;">
                            <div class="erp-toggle-dot absolute bg-white rounded-full shadow" style="width:16px;height:16px;top:2px;left:2px;transition:transform 0.2s;transform:translateX(0);"></div>
                        </div>
                        <div>
        ...
        <div class="p-5 border-t border-zinc-200 flex items-center justify-between">
            ...
            <button type="button" id="save" data-route="{{ route('env-variable.store') }}"
                ...
                <i class="fa-solid fa-check text-xs"></i> {{ __('lang.common.save') }}
            </button>
        </div>
    </div>
    </div>
...
$(document).ready(function() {
    ...
    // Form submission handled by global save.js (.save class + data-route)
});
</script>
@endsection
