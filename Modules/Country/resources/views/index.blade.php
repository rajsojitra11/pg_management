...
@section('title', __('country::message.list'))
@section('nav-module', 'country')
@section('breadcrumb', 'Home > Masters > Country')

@section('content')
{{-- Page Header --}}
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-xl sm:text-2xl font-bold text-zinc-900">{{ __('country::message.list') }}</h1>
    </div>
...
<div id="inlineModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="relative flex items-center justify-center min-h-screen p-4">
        <div class="relative w-full max-w-sm rounded-lg border bg-white shadow-xl" style="border-color: var(--erp-border); background-color: var(--erp-bg);">
            {{-- Header --}}
            <div class="flex items-center justify-between p-4 border-b" style="border-color: var(--erp-border);">
                <h3 class="text-lg font-semibold" id="modalTitle" style="color: var(--erp-text);">{{ __('country::message.add') }}</h3>
            ...
            <div id="body">
                <form id="form" action="javascript:void(0);" method="POST" novalidate>
                    ...
                    <div class="p-4 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" required
                                   ...
                                   name="name" id="name"
                                   placeholder="{{ __('country::message.enter_name') }}">
                        ...
                        <div>
                            ...
                            <input type="text" required maxlength="10"
                                   ...
                                   style="border-color: var(--erp-border); background-color: var(--erp-bg); color: var(--erp-text); --tw-ring-color: var(--erp-primary);"
                                   name="code" id="code"
                                   placeholder="{{ __('country::message.code') }}">
                    ...
                    <div class="flex items-center justify-end gap-2 p-4 border-t" style="border-color: var(--erp-border);">
                        ...
                        <button type="button" id="save" data-route="{{ route('country.store') }}"
...
<script type="application/javascript">
    'use strict';
    window.URL_ROUTE = "{{ route('country.index') }}";
    ...
    function resetInlineModal() {
        ...
        $("#save").attr('data-route', "{{ route('country.store') }}")
        ...
        $("#modalTitle").html("{{ __('country::message.add') }}");
    ...
    $(document).on('click', '.edit', function(e) {
        ...
        $("#save").attr('data-route', "{{ route('country.update', ':id') }}".replace(':id', id));
        $.ajax({
            type: "GET",
            url: url,
            dataType: 'json',
            success: function(response) {
                if (response.status_code == 200) {
                    $("#modalTitle").html("{{ __('country::message.edit') }}");
