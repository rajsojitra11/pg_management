...
@section('title', __('report::message.maintenance_report'))
@section('nav-module', 'report')
@section('breadcrumb', 'Home > Reports > Maintenance Report')

@section('content')
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-xl sm:text-2xl font-bold text-zinc-900">{{ __('report::message.maintenance_report') }}</h1>
...
<form id="filter_form" class="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm mb-4" onsubmit="return false;">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:items-end">
        <div class="lg:col-span-3">
            ...
            <div class="flex h-9 rounded-md border border-zinc-200 bg-white focus-within:ring-2 focus-within:ring-zinc-900 focus-within:ring-offset-2 overflow-hidden">
                ...
                <input type="text" id="filterSearch" name="filter_search" placeholder="{{ __('report::message.maintenance_search') }}" class="flex-1 min-w-0 bg-transparent px-3 text-sm text-zinc-700 placeholder:text-zinc-400 focus:outline-none">
            </div>
        </div>
        ...
        <div class="lg:col-span-2">
            ...
            <select id="filterStatus" name="filter_status" class="h-9 w-full rounded-md border border-zinc-200 bg-white px-3 text-sm text-zinc-700 focus:ring-1 focus:ring-zinc-500 focus:border-zinc-500">
                ...
                <option value="pending">{{ __('report::message.status_pending') }}</option>
                <option value="in_progress">{{ __('report::message.status_in_progress') }}</option>
                <option value="completed">{{ __('report::message.status_completed') }}</option>
            </select>
        </div>
        <div class="lg:col-span-2">
    ...
    <div class="flex items-center gap-2 justify-end mt-3">
        ...
        <button type="button" class="excel h-9 px-3 rounded-md bg-green-700 text-white text-sm font-medium hover:bg-green-800" data-export-url="{{ route('report.maintenance.export') }}"><i class="fa-solid fa-file-excel mr-1"></i> {{ __('report::message.excel') }}</button>
    </div>
</form>
    ...
    <div class="p-4 overflow-x-auto">
        <table id="table" class="display responsive nowrap w-full">
            <thead>
                <tr>
                    <th>#</th>
                    <th>{{ __('report::message.maintenance_no') }}</th>
                    <th>{{ __('report::message.complaint_no') }}</th>
                    <th>{{ __('report::message.room_no') }}</th>
                    <th>{{ __('report::message.maintenance_date') }}</th>
                    <th>{{ __('report::message.description') }}</th>
                    <th>{{ __('report::message.cost') }}</th>
    ...
    $(function() {
        ...
        table = initErpTable('#table', {
            ...
            columns: [
                ...
                { data: 'maintenance_date', name: 'maintenance_date', render: function(data) { return window.erpDate ? window.erpDate(data) : (data || '-'); } },
                { data: 'description', name: 'description', render: function(data) { return data || '-'; } },
                { data: 'cost', name: 'cost', render: function(data) { return data ? '₹' + parseFloat(data).toFixed(2) : '-'; } },
                { data: 'status', name: 'status', render: function(data) { var label = (data || '').replace(/_/g, ' '); return window.erpBadge ? window.erpBadge(label) : (label || '-'); } }
            ]
        });
