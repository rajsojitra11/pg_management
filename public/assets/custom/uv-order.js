...
(function () {
    ...
    const categorySlug = 'uv';
    const prefillPath = 'uv-prefill';
    const initial = window.LO_INITIAL || null;
    const resourceRoute = (initial && initial.resource_route) || 'uvorders';
    ...
    // In create mode, hide order forms where all UV types have already been
    // added, so only those with pending types still available appear.
    const orderExclude = mode === 'create' ? { exclude_with_all_uv: 1 } : {};
    ...
    $('#loSubmit').on('click', function () {
        ...
        if (singleItemId) {
            // Per-item save: update just this one item in-place.
            const row = items[0] || {};
            const payload = {
